...
function installOrientationGuard() {
    ...
    const update = () => {
        ...
        if (!coarse)
            return guard.classList.remove('show');
        const shortSide = Math.min(window.innerWidth, window.innerHeight);
        const isPhone = shortSide < 600;
        const isPortrait = window.innerHeight >= window.innerWidth;
        guard.classList.toggle('show', isPhone ? !isPortrait : isPortrait);
        guard.querySelector('.orientation-title').textContent = isPhone ? 'Rotate to portrait' : 'Rotate to landscape';
        guard.querySelector('.orientation-copy').textContent = isPhone
