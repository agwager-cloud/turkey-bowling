...
export class LiveSpectatorScene extends BaseBowlingScene {
    ...
    render(state) {
      ...
      <div class="bowling-shell interactive realistic-bowling-shell live-spectator-shell${match.bowlOffActive ? ' bowl-off-shell' : ''}">
        ...
        else if (mathGame && mathFrame !== undefined)
    ...
    async playSpectatorShot(shot) {
        ...
        if (note)
            note.textContent = `LIVE — ${bowler.name.toUpperCase()} BOWLING`;
        if (speed)
            speed.textContent = 'BALL IN MOTION…';
        if (status)
            status.textContent = `${bowler.name} has released the ball — this is the exact live attempt.`;
        ...
        const showCelebration = () => {
            if (celebrationShown || !anticipatedCelebration || !this.scene.isActive() || !this.ui || this.spectatorPlayerId !== shot.playerId)
                return;
        ...
        if (!result.gutter && result.knockedPins.length > 2 && !impactPromise)
            playImpact();
...
function renderGlobalSpectatorPanel(player, game, match, level, mathFrame) {
    if (match.complete)
        return `<div class="spectator-turn-panel quiet"><strong>MATCH COMPLETE</strong><span>Returning to Class Matchups…</span></div>`;
    if (!player || !game)
...
function playerInMatch(match, playerId) {
    if (!playerId)
        return null;
...
function watchedMatchFingerprint(state, matchId) {
...
function frameStatus(game) {
    if (game.complete && game.pendingMathFrames.length)
        return 'All 10 frames are complete • score checks still in progress.';
    if (game.complete)
        return `10-frame game complete — score ${game.total ?? game.finalScore ?? 0}.`;
