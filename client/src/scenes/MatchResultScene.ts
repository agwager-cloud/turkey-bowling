...
export class MatchResultScene extends BaseBowlingScene {
    ...
    render(state, roundResult) {
      ...
      <div class="result-shell interactive">
        ...
        this.ui.querySelector('#result-matchups')?.addEventListener('click', () => this.scene.start('MatchupScene'));
        this.ui.querySelector('#result-class-matchups')?.addEventListener('click', () => this.scene.start('MatchupScene'));
    }
}
...
function laneMovementText(oldLane, newLane, championship) {
    if (newLane > oldLane)
        return `⬆ PROMOTED: Lane ${oldLane} → ${newLane}${championship ? ' • Championship defended!' : ''}`;
    if (newLane < oldLane)
        return `⬇ RELEGATED: Lane ${oldLane} → ${newLane}`;
    if (championship)
        return '👑 Staying on the Championship Lane';
    return `↔ Staying on Lane ${oldLane}`;
}
function renderFinalScoreSide(name, raw, finalScore, timeouts, penaltyPercent, me) {
